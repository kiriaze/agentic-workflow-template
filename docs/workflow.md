...
- Design constraints live in `PRODUCT.md` and `DESIGN.md` — run `impeccable teach` once per project to generate them.
...
docs/AGENT_TASKS.md              ← Live task registry. CC writes rows, Codex executes spec files.
docs/tasks/TASK-NNN.md           ← Individual task spec files (one per active task; deleted on merge).
...
docs/agent-roster.json           ← Model assignments by role, effort, and task type.
docs/plans/planning-template.md  ← Template for Orchestrator → Implementer handoffs.
docs/plans/example-plan.md       ← Filled-in example plan.
...
PRODUCT.md                       ← Brand, users, principles. Generated by `impeccable teach`.
DESIGN.md                        ← Visual tokens, typography, components. Generated by `impeccable teach`.
```
