...
- **Read `PRODUCT.md` and `DESIGN.md`** before any significant UI work — generated by `impeccable teach` or an equivalent design brief tool.
...
0. **If `.scaffold-backups/` exists** (upgrade) — `setup.sh` created `docs/tasks/TASK-000-adapt-scaffold.md` automatically. The session-start hook injects it; execute it before anything else. Claude reads the backups, fills in all `[fill in]` placeholders, creates `.scripts/quality-gates.sh` for the detected stack, and reports what was migrated. Skip on fresh projects.
...
5. For UI/design work: confirm `PRODUCT.md` exists before starting — run `impeccable teach` (or equivalent) if missing.
...
- New feature or component → invoke `impeccable:shape` (or equivalent design brief) before writing code.
...
| Trigger | Invoke |
|---|---|
| After any non-trivial code change | `simplify` (or ask "review this diff for unnecessary complexity") |
| After a significant UI component | `impeccable:audit` (or equivalent UI audit) |
